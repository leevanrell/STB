...
def main():
	df = getDF()
	RNN_df = df.drop(['EMA20', 'SMA20', 'SMA50', 'SMA200', 'RSI', 'MACD', 'views', 'one', 'volume'], axis=1)
...
def getDF():
	df1 = getStock(Stock_file, 'AMD') # get stock and indicators
	df2 = getViews(Wiki_file, 'AMD') # get wikipedia views
	result = df2.join(df1) # combine data set

	result = normalize(result, 'views') # normalize data

	result = normalize(result, 'close')
	#result = replaceNaN(result, 'close') # replace weekend with friday values

	result = normalize(result, 'volume')
	result = replaceNaN(result, 'volume') # replace weekend with friday values

	result = getOneDay(result, 'close')
	print(result.head(10))
	return result


def getStock(Stock_db, Company):
	conn = sqlite3.connect(Stock_db, timeout=7)
	c = conn.cursor()
...
def getViews(Views_db, Company):
	conn = sqlite3.connect('./data/wiki.db', timeout=7)
	c = conn.cursor()
...
def graph(df):
	...
	y2 = np.array(df['views'])
	y3 = np.array(df['volume'])
	...
	plt.legend(['Price', 'Wikipedia Views', 'Volume'])
	plt.show()
